...
void PrintClockCycle(int type, unsigned long long clockcycle);
	       ...
	       *result = fopen("result.txt", "w"),
	        *insert = fopen("IPv4_insert.txt", "r"),
	         *delete = fopen("IPv4_delete.txt", "r"),
	          *search_clockcycle = fopen("search_clockcycle.csv", "w"),
	           *insert_clockcycle = fopen("insert_clockcycle.csv", "w"),
	            *delete_clockcycle = fopen("delete_clockcycle.csv", "w");
...
//LPM Search
	...
	{
		unsigned long long begin, end, clockcycle = 0;
		begin = rdtsc();
		...
		if (group25_32[element] != NULL) //means first 12 significant bits match ip_32bits
		{
			...
			if (ip_search->length != 0) //means it is not default prefix
			{
				...
				clockcycle = end - begin;
				PrintClockCycle(1, clockcycle);
...
//Delete Node
	...
	for (i = 0; i < 1000; i++)
	{
		fprintf(search_clockcycle, "%d\n", times[0][i]);
		fprintf(insert_clockcycle, "%d\n", times[1][i]);
		fprintf(delete_clockcycle, "%d\n", times[2][i]);
	}
	fclose(prefix);
	fclose(search);
	fclose(result);
	fclose(insert);
	fclose(delete);
	fclose(search_clockcycle);
	fclose(insert_clockcycle);
	fclose(delete_clockcycle);
	return 0;
}
...
void PrintClockCycle(int i, unsigned long long clockcycle)
{
	if (clockcycle < 35000)
		times[i - 1][clockcycle / 35]++;
}
