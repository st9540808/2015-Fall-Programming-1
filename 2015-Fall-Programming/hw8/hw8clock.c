...
void PrintClockCycle(FILE *fp, unsigned long long clockcycle);
	              ...
	              &ip[0], &ip[1], &ip[2], &ip[3]) != EOF)
	{
		...
		if (group25_32[element] != NULL) //means first 12 significant bits match ip_32bits
		{
			...
			if (ip_search->length != 0) //means it is not default prefix
			{
				...
				PrintClockCycle(search_clockcycle, clockcycle);
...
//Insert
	              ...
	              &ip[0], &ip[1], &ip[2], &ip[3], &length) != EOF)
	{
		...
		if (length >= 8 && length < 16) //insert into group8_15
		{
			...
			clockcycle = end - begin;
			PrintClockCycle(insert_clockcycle, clockcycle);
		}
...
//Delete Node
	              ...
	              &ip[0], &ip[1], &ip[2], &ip[3], &length) != EOF)
	{
		...
		if (length >= 8 && length < 16)
		{
			...
			clockcycle = end - begin;
			PrintClockCycle(delete_clockcycle, clockcycle);
		}
